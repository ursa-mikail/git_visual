...
import {
  ...
  getCrossDiff, getRefTree, getCrossRefTree,
} from '../utils/api'
...
function buildTree(paths: string[], statusMap: Map<string, { status: DiffStatus; index: number }>): TreeNode {
...
function hasDiffChild(n: TreeNode): boolean {
  return n.children.some(c => c.isDir ? hasDiffChild(c) : !!c.diffStatus)
}
function containsIdx(n: TreeNode, idx: number | null): boolean {
  if (idx == null) return false
  return n.children.some(c => c.isDir ? containsIdx(c, idx) : c.diffIndex === idx)
}

function TreeNode({ node, depth, selIdx, onSelect, open, toggle }: {
  node: TreeNode; depth: number; selIdx: number | null
  onSelect: (i: number) => void; open: Set<string>; toggle: (p: string) => void
}) {
  if (node.isDir && !node.name) {
    return <>{node.children.map(c => <TreeNode key={c.path} node={c} depth={depth} selIdx={selIdx} onSelect={onSelect} open={open} toggle={toggle} />)}</>
  }
  const indent = depth * 12

  if (node.isDir) {
    const isOpen   = open.has(node.path)
    const active   = containsIdx(node, selIdx)
    const hasChg   = hasDiffChild(node)
    return (
      <div>
        ...
        {isOpen && node.children.map(c =>
          <TreeNode key={c.path} node={c} depth={depth + 1} selIdx={selIdx} onSelect={onSelect} open={open} toggle={toggle} />
        )}
      </div>
    )
  }
  ...
  return (
    <div onClick={() => node.diffIndex != null && onSelect(node.diffIndex)} style={{
      display: 'flex', alignItems: 'center', gap: 4,
      padding: '2px 6px', paddingLeft: 4 + indent,
      cursor: hasDiff ? 'pointer' : 'default', fontSize: 11, userSelect: 'none',
      background: isSel ? 'rgba(59,130,246,.12)' : 'transparent',
      color, opacity: hasDiff ? 1 : 0.45,
    }}
      onMouseEnter={e => { if (hasDiff && !isSel) e.currentTarget.style.background = 'var(--bg-hover)' }}
      onMouseLeave={e => { if (!isSel) e.currentTarget.style.background = 'transparent' }}>
      {node.diffStatus ? S_ICON[node.diffStatus] : <File size={11} color="var(--text-muted)" />}
      <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: hasDiff ? 500 : 400 }}>
        {node.name}
      </span>
    </div>
  )
...
interface PickerOption { label: string; ref: string; repoId: string; repoName: string; kind: string }
...
function RefPicker({ options, value, repoIdVal, onSelect, placeholder, accentColor }: {
  ...
  const filtered = q
    ? options.filter(o => o.label.toLowerCase().includes(q) || o.ref.toLowerCase().includes(q) || o.repoName.toLowerCase().includes(q))
    : options

  // Group by repoName + kind
  ...
  const displayLabel = selected
    ? (selected.repoName !== 'this' ? selected.repoName + '/' : '') + selected.ref
    : placeholder
    ...
    <div ref={ref} style={{ position: 'relative', display: 'inline-block', minWidth: 240 }}>
      ...
      {open && (
        <div style={{
          ...
          width: 340, maxHeight: 400, overflow: 'hidden',
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {/* Clear option */}
            <div onClick={() => { onSelect('', ''); setOpen(false); setQuery('') }}
              ...
              groups.filter(g => g.kind === kind).map(group => (
                <div key={group.key}>
                  <div style={{
                    ...
                    {group.repoName !== 'this' && (
                      <span style={{ background: 'var(--accent-blue-dim, rgba(59,130,246,.2))', color: 'var(--accent-blue)',
                  ...
                  {group.items.map(o => {
                    ...
                    // For commits, show hash prominently + message separately
                    const isCommit = o.kind === 'commit'
                    const hashPart = isCommit ? o.ref : null
                    const msgPart  = isCommit ? o.label.replace(o.ref, '').trim() : o.label
                    return (
                      <div key={o.repoId + '|' + o.ref}
                        ...
                        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                          fontSize: isCommit ? 11 : 12, color: isCommit ? 'var(--text-secondary)' : 'inherit' }}>
                          {msgPart}
                        </span>
                      </div>
                    )
...
export default function DiffPage() {
  ...
  // ── Status maps ───────────────────────────────────────────────────────────
  // Classify each file's status from the perspective of each side
  const baseStatusMap = useMemo(() => {
    ...
    files.forEach((f, i) => {
      ...
      const status: DiffStatus = isRename ? 'renamed'
        : f.deletions > 0 && f.additions === 0 ? 'removed'
        : f.additions > 0 && f.deletions === 0 ? 'added'
        : 'modified'
      if (!m.has(p)) m.set(p, { status, index: i })
    })
    return m
  }, [files])
  ...
  const cmpStatusMap = useMemo(() => {
    ...
    files.forEach((f, i) => {
      ...
      const status: DiffStatus = isRename ? 'renamed'
        ...
        : f.additions > 0 && f.deletions === 0 ? 'added'
        : 'modified'
      if (!m.has(f.path)) m.set(f.path, { status, index: i })
    })
    return m
  }, [files])

  // ── Trees (pure memo, no side-effects) ────────────────────────────────────
  const baseTree = useMemo(() => {
    const pathSet = new Set((baseFileList as FileEntry[]).map(f => f.path))
    // add any diff paths not yet in file list
    files.forEach(f => { const p = f.old_path || f.path; if (!pathSet.has(p)) pathSet.add(p) })
    return buildTree([...pathSet], baseStatusMap)
  }, [baseFileList, baseStatusMap, files])

  const cmpTree = useMemo(() => {
    const pathSet = new Set((cmpFileList as FileEntry[]).map(f => f.path))
    files.forEach(f => { if (!pathSet.has(f.path)) pathSet.add(f.path) })
    return buildTree([...pathSet], cmpStatusMap)
  }, [cmpFileList, cmpStatusMap, files])

  // ── Auto-expand dirs with diffs; auto-select first changed file ───────────
  useEffect(() => {
    const expand = (node: TreeNode) => {
      const paths = new Set<string>()
      const walk  = (n: TreeNode) => {
        if (n.isDir && n.name && hasDiffChild(n)) paths.add(n.path)
  ...
  const ownRefs    = arl.filter(r => r.repo_id === repoId)
  const otherRefs  = arl.filter(r => r.repo_id !== repoId)
  const otherRepos = [...new Set(otherRefs.map(r => r.repo_id))]

  // Build PickerOption arrays for RefPicker
  const simpleOptions: PickerOption[] = [
    ...local.map(r   => ({ label: r.label, ref: r.ref, repoId: repoId!, repoName: 'this', kind: r.kind })),
    ...remote.map(r  => ({ label: r.label, ref: r.ref, repoId: repoId!, repoName: 'this', kind: r.kind })),
    ...tags.map(r    => ({ label: r.label, ref: r.ref, repoId: repoId!, repoName: 'this', kind: r.kind })),
    ...commits.map(r => ({ label: r.label, ref: r.ref, repoId: repoId!, repoName: 'this', kind: r.kind })),
  ]

  // Build repo name map for cross picker labels
  const repoNameMap = new Map<string, string>([[repoId!, 'this']])
  otherRepos.forEach(rid => {
    const first = otherRefs.find(r => r.repo_id === rid)
    if (first) repoNameMap.set(rid, first.label.split('/')[0])
  })
  ...
  const Explorer = ({
    label, color, tree, open, toggle,
    border,
  }: {
    label: string; color: string; tree: TreeNode
    ...
    <div style={{
      ...
      [border === 'left' ? 'borderRight' : 'borderLeft']: '1px solid var(--border)',
    }}>
      <div style={{
        padding: '5px 8px', fontSize: 10, fontWeight: 700, color,
        letterSpacing: '0.07em', textTransform: 'uppercase',
        borderBottom: '1px solid var(--border)', flexShrink: 0,
        background: 'var(--bg-surface)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
      }}>
        {label}
      </div>
      <div style={{ overflowY: 'auto', flex: 1, paddingTop: 4 }}>
        <TreeNode node={tree} depth={0} selIdx={selIdx} onSelect={setSelIdx} open={open} toggle={toggle} />
      </div>
    </div>
  )
  ...
  return (
    <div style={{ padding: 24, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      {debugMode && hasRef && !working && (
        <div style={{ marginBottom: 10, padding: 10, background: 'var(--bg-elevated)',
          ...
          <div style={{ fontWeight: 700, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-secondary)' }}>
            ...
            <button className="btn btn-ghost btn-sm" style={{ marginLeft: 'auto' }}
              onClick={() => refetchDebug()} disabled={debugLoading}>
      ...
      {diffLoading ? (
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden', border: '1px solid var(--border)', borderRadius: 8, minHeight: 0 }}>
          ...
          <Explorer
            ...
            tree={baseTree} open={baseOpen} toggle={toggleBase} border="left"
          />
          ...
          <div style={{ flex: 1, overflowY: 'auto', minWidth: 0 }}>
            {files.length === 0 ? (
              ...
              <div className="empty-state" style={{ padding: 40 }}>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Dimmed files are unchanged</p>
              </div>
            ) : (
              <>
                <div style={{
          ...
          <Explorer
            ...
            tree={cmpTree} open={cmpOpen} toggle={toggleCmp} border="right"
          />
        </div>
      )}
    </div>
...
function SideBySide({ file }: { file: DiffFile }) {
  ...
  const Panel = ({ rows, fp, stat }: { rows: Row[]; fp: string; stat: React.ReactNode }) => (
    <div style={{ overflow: 'auto', borderRight: '1px solid var(--border)' }}>
      <div style={{ padding: '2px 8px', background: 'var(--bg-elevated)', fontSize: 11, color: 'var(--text-muted)',
        fontWeight: 600, borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, position: 'sticky', top: 0 }}>
        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis' }}>{fp}</span>{stat}
      </div>
      {rows.map((row, i) => (
        <div key={i} style={{ display: 'flex', fontFamily: 'var(--font-mono)', fontSize: 12, minHeight: 20, background: bg(row.type) }}>
          <span style={{ minWidth: 38, padding: '0 5px', textAlign: 'right', color: 'var(--text-muted)',
            borderRight: '1px solid var(--border-muted)', fontSize: 11, userSelect: 'none', flexShrink: 0 }}>
            {row.num ?? ''}
          </span>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderTop: '1px solid var(--border)' }}>
      <Panel rows={L} fp={file.old_path || file.path} stat={<span style={{ color: 'var(--accent-red-hover)' }}>-{file.deletions}</span>} />
      <Panel rows={R} fp={file.path} stat={<span style={{ color: 'var(--diff-add-text)' }}>+{file.additions}</span>} />
    </div>
  )
}
